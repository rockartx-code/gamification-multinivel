...
def _plantillas(order: dict, evento: str, datos: dict, frontend_url: str):
    ...
    if evento == "paid":
        ...
        # Paquete D · propuesta 39: los plazos no se escriben a mano en el correo
        # (decían 48 h y 7 días aunque la configuración dijera otra cosa) y el
        # proceso completo se publica **antes** de que haga falta.
        plazos = " ".join(f"«{m['label']}»: {m['plazoTexto']}."
                          for m in _motivos_publicados_seguro())
        lead = ("Revisa que todo esté bien. Si algo salió mal puedes pedir la devolución desde tu "
